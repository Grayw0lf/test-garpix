...
from ticket.helpers import user_is_author, user_is_performer, can_processed
...
class Ticket(models.Model):
    ...
    performer = models.ForeignKey(get_user_model(),
    ...
    def assigned(self):
        ...
        users = get_user_model().objects.prefetch_related(
            Prefetch(self.ASSIGNED_TICKETS_NAME,
                     self.__class__.objects.filter(state__in=('assigned',
                                                              'processed')))
    ...
    @transition(field='state', source='assigned', target='processed',
                permission=user_is_performer, conditions=[can_processed])
    def processed(self):
        """
    ...
    @transition('state', source='processed', target='successed',
                permission=user_is_performer)
    def successed(self):
        """Выполнение тикета"""

    @transition('state', source='processed', target='canceled',
                permission=user_is_performer)
    def canceled(self):
        """Отмена тикета"""

    @transition('state', source=('canceled', 'successed'),
                target='closed', permission=user_is_author)
    def closed(self):
        """Закрытие тикета"""

    @transition('state', source=('canceled', 'successed'),
                target='reopened', permission=user_is_author)
    def reopened(self):
        """Переоткрытие тикета"""
